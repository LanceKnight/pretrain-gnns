...
def train(args, model_list, loader, optimizer_list, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        # print(f'pred_node:{pred_node.shape}')
        # print(f'mask_node_label:{batch.mask_node_label.shape}')
        # print(f'mask_node_label:{batch.mask_node_label[:,0:5]}')
        # loss = criterion(pred_node.double(), batch.mask_node_label[:, 0])
        loss = criterion(pred_node, batch.mask_node_label[:, 0:5])
...
def main():
    ...
    parser.add_argument('--output_model_file', type=str,
                        default='', help='filename to output the model')
    ...
    dataset = MoleculeDataset("dataset/" + args.dataset, dataset=args.dataset, transform=MaskAtom(
        num_atom_type=119, num_edge_type=5, mask_rate=args.mask_rate, mask_edge=args.mask_edge))
    ...
    linear_pred_atoms = torch.nn.Linear(args.emb_dim, 5).to(device)
    ...
    optimizer_linear_pred_atoms = optim.Adam(
        linear_pred_atoms.parameters(), lr=args.lr, weight_decay=args.decay)
    optimizer_linear_pred_bonds = optim.Adam(
        linear_pred_bonds.parameters(), lr=args.lr, weight_decay=args.decay)

    optimizer_list = [optimizer_model,
                      optimizer_linear_pred_atoms, optimizer_linear_pred_bonds]
